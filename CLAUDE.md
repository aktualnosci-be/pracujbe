...
- [x] Warstwa danych paneli bez PostgREST (#25): loadery/akcje/layouty/onboarding/outbox na `withPortalTransaction`
  ...
  (`claim_email_batch` dla `service_role`). Dowód: `tests/integration/portal-*.test.ts` (PG16). Nazwa firmy z rejestracji
  w formularzu firmy — zrobione (Etap 3, `readSignupCompanyName`). **Otwarte:** nazwa firmy w wiadomościach
  kandydata (od 0014);
