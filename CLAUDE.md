...
- [x] CSP (P2-01) — `next.config.mjs` (default/object/frame-ancestors/base/form-action + zawężone
  ...
  Report-Only (#585, część): produkcja (poza dev) wysyła obok egzekwowanej polityki
  `Content-Security-Policy-Report-Only` z tym samym `script-src`, ale bez `'unsafe-inline'` —
  skrypt banera zgód dopuszczony hashem sha256 liczonym w `next.config.mjs` z tej samej treści
  co render (`src/lib/security/csp-inline-scripts.mjs`, domyślna wersja polityki zgód = `2.0`),
  beacon Cloudflare (zewnętrzny `src`) hostem tylko przy tokenie. Nic nie blokuje, raporty na
  `/api/csp-report` (oczekiwany szum: własne skrypty RSC Next.js). Egzekwowany `script-src`
  zostaje z `'unsafe-inline'`. Dowód: `csp-inline-scripts.test` (kontrola ujemna: inna treść
  skryptu / inna wersja polityki = brak hasha).
