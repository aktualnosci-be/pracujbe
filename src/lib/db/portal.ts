...
import { isPortalAuthConfigured, isServiceDatabaseConfigured } from '@/lib/env';
 ...
 *
 * - `getPortalIdentity()` — tożsamość z sesji serwerowej (`getCurrentIdentity` z #24: Better Auth →
...
export function isPortalDataConfigured(): boolean {
  return isPortalAuthConfigured();
}

async function readIdentity(): Promise<PortalIdentity | null> {
  if (!isPortalDataConfigured()) return null;
  // Jedno źródło tożsamości (#24): `getCurrentIdentity` — sesja Better Auth → aktywny profil.
  const { getCurrentIdentity } = await import('@/lib/auth/current');
