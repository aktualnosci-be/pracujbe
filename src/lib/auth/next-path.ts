...
/**
 * Bezpieczny adres powrotu po logowaniu/rejestracji (`?next=`) i linki logowania/rejestracji.
 *
 * Moduł celowo NIE importuje Zoda ani schematów (`@/lib/validation/auth`): używają go komponenty
 * klienckie obecne na każdej stronie publicznej (przełącznik języka, zapis oferty, ApplyModal),
 * więc każdy import tutaj trafia do ich bundla (#390). Strażnik: `tests/unit/public-bundle-no-zod.test.ts`
 * i `scripts/check-next-build.mjs`.
 */
