 ...
 *     receipty regulaminu i informacji o prywatności (`record_signup_consents`, 0107); bez `finish`
 ...
 * Kluczowane po zweryfikowanym `user.id` z sesji (RPC tylko service_role). Best-effort jak
...
async function recordTermsAcceptance(profileId: string): Promise<void> {
  try {
    ...
    const { error } = await createAdminClient().rpc('record_signup_consents', {
      ...
      p_source: 'onboarding',
      p_locale: locale,
      p_wording_versions: consentWordingVersions('onboarding', locale),
      p_ip: ip,
      p_user_agent: store.get('user-agent'),
    });
    if (error) captureError(error, { area: 'onboarding.recordSignupConsents' });
  } catch (e) {
