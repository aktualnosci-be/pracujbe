...
export async function setCompanyStatus(
  ...
  if (!companyId || !(COMPANY_STATUSES as readonly string[]).includes(status)) {
...
export async function resolveReport(reportId: string, status: string): Promise<AdminActionResult> {
  if (!reportId || !(REPORT_STATUSES as readonly string[]).includes(status)) {
    return { ok: false, error: 'VALIDATION_FAILED' };
  }
