...
export async function getCandidateFiles(): Promise<CandidateFile[]> {
  if (!isPortalDataConfigured()) return [];
  try {
    const { getSignedFileUrl } = await import('@/lib/storage');
    const me = await getPortalIdentity();
    if (!me) return [];

    const rows = await withPortalTransaction(me, (tx) => queryRows(tx, 'candidate.files',
      `SELECT id, path, bucket, file_name
         FROM public.files
        WHERE owner_id = $1 AND entity_type = 'candidate_cv' AND deleted_at IS NULL
        ORDER BY created_at DESC`, [me.id]));
    // Signed URL (Storage, #26) poza transakcją bazy.
    return Promise.all(
      rows.map(async (row) => {
        const r = asRecord(row);
