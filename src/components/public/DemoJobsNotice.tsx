 ...
 * Strona decyduje o wyświetleniu (`isShowingDemoJobs()` lub `job.isDemo`).
 */
export async function DemoJobsNotice({ className }: { className?: string }): Promise<React.JSX.Element> {
  ...
  return (
    <aside
      ...
      className={cn(
        'flex items-start gap-3 rounded-2xl border border-warning/40 bg-warning/10 p-4 text-sm text-foreground',
        className,
      )}
    >
