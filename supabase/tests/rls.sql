...
-- ============================================================================
-- PL113. Payloady e-maili i odczyt historii (0113; #293, #22, #290, #184):
...
insert into public.companies(id,name,status) values (:'PLCO','Firma PL113','verified');
...
insert into public.jobs(id,company_id,slug,title,category,contract_type,city,region,status,default_locale,
  ...
  (:'PLJ1',:'PLCO','job-pl113-1','Magazynier PL113','warehouse','permanent','Gent','Flandria','active','pl',
   2500,3100,'EUR','month', now() + interval '10 days'),
  (:'PLJ2',:'PLCO','job-pl113-2','Kierowca PL113','warehouse','permanent','Gent','Flandria','active','pl',
   null,null,'EUR','hour', null),
  (:'PLJ3',:'PLCO','job-pl113-3','Pomocnik PL113','warehouse','permanent','Gent','Flandria','active','pl',
...
set role authenticated; select pg_temp.assert_client_role();
select public.apply_to_job(:'PLJ1'::uuid, 'pl113-app-1', null, null, null) as plapp1 \gset
select public.apply_to_job(:'PLJ2'::uuid, 'pl113-app-2', null, null, null) as plapp2 \gset
reset role;
select set_config('app.current_uid', :'PLC2', false);
set role authenticated; select pg_temp.assert_client_role();
select public.apply_to_job(:'PLJ3'::uuid, 'pl113-app-3', null, null, null) as plapp3 \gset
reset role;
select set_config('app.current_uid', :'PLE', false);
set role authenticated; select pg_temp.assert_client_role();
select public.send_offer(:'PLJ1'::uuid, :'PLC'::uuid, 'pl113-off-1', 'Bel me op 0470 12 34 56', null) as ploff1 \gset
select public.send_offer(:'PLJ2'::uuid, :'PLC'::uuid, 'pl113-off-2', null, null) as ploff2 \gset
...
-- PL113-1: termin = offers.expires_at (ten, który sprawdza respond_to_offer) jako ISO.
select pg_temp.assert(
  ...
  'PL113-1 jobOffer.expiresAt = offers.expires_at');
-- PL113-2: kwoty jako liczby + okres i waluta (tekst składa worker w locale odbiorcy).
select pg_temp.assert(
  ...
  'PL113-2 jobOffer niesie kwoty oferty, bez gotowego tekstu wynagrodzenia');
-- PL113-3: oferta bez kwot → null (worker pomija pole), okres z danych.
select pg_temp.assert(
  ...
  'PL113-3 oferta bez kwot: null w payloadzie, termin domyślny (+30 dni) obecny');
-- PL113-4: treść wiadomości rekrutera zostaje w offers, NIE trafia do payloadu (#503).
select pg_temp.assert(
  ...
  and (select not payload ? 'message' and payload::text not like '%0470%'
         from public.email_deliveries where entity_id = :'ploff1' and template = 'jobOffer'),
  'PL113-4 payload jobOffer bez treści wiadomości rekrutera');
-- PL113-5: język e-maila = język ODBIORCY (nl), nie nadawcy (pl) — Invariant #1.
select pg_temp.assert(
  (select locale from public.email_deliveries where entity_id = :'ploff1' and template = 'jobOffer') = 'nl',
  'PL113-5 jobOffer w języku kandydata');
-- PL113-6: newMessage niesie identyfikator rozmowy (CTA do wątku).
select pg_temp.assert(
  (select payload->>'conversationId' = :'plconv' and locale = 'nl'
     from public.email_deliveries where entity_id = :'plmsg' and profile_id = :'PLC'),
  'PL113-6 newMessage.conversationId = rozmowa wiadomości');

-- PL113-7: get_applied_jobs_display — filtr p_job_ids wewnątrz RPC, tylko własne aplikacje.
...
select pg_temp.assert(
  ...
  'PL113-7f stary podpis usunięty; EXECUTE tylko authenticated');

-- KONTROLA UJEMNA 1: send_offer bez expiresAt → asercja PL113-1 wykrywa brak.
begin;
do $pl$ begin
...
select pg_temp.assert(pg_get_functiondef('public.send_offer(uuid, uuid, text, text, timestamptz)'::regprocedure)
  not like '%expiresAt%', 'PL113-N1 mutacja usunęła klucz expiresAt');
select set_config('app.current_uid', :'PLE', false);
set local role authenticated; select pg_temp.assert_client_role();
select public.send_offer(:'PLJ3'::uuid, :'PLC2'::uuid, 'pl113-off-neg', null, null) as ploffneg \gset
reset role;
select pg_temp.assert(
  ...
  'PL113-N1b bez klucza predykat PL113-1 jest fałszywy (test wykrywa regresję)');
rollback;
reset role; reset app.current_uid;

-- KONTROLA UJEMNA 2: send_message bez conversationId → asercja PL113-6 wykrywa brak.
begin;
do $pl$ begin
...
select pg_temp.assert(pg_get_functiondef('public.send_message(uuid, text, uuid)'::regprocedure)
  not like '%''conversationId''%', 'PL113-N2 mutacja usunęła klucz conversationId');
select set_config('app.current_uid', :'PLE', false);
set local role authenticated; select pg_temp.assert_client_role();
...
select pg_temp.assert(
  ...
  'PL113-N2b bez klucza predykat PL113-6 jest fałszywy (test wykrywa regresję)');
rollback;
reset role; reset app.current_uid;
