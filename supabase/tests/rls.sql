...
reset role; reset app.current_uid;
-- CL162 (0162) dodał CHECK website/logo_url (bezwzględny https) NA POZIOMIE TABELI — ten test
...
-- (nowe zapisy nadal walidowane — CL162 niżej to sprawdza — bez skanowania tych wierszy).
...
-- ============================================================================
-- CL162. Edycja strony WWW i logo firmy (#112, 0162): tylko owner/admin (jak
...
--        nazwy/VAT — `protect_company_verification`, 0072/0162 bez zmian); audyt
...
-- CL162-1 (kontrola ujemna): member/recruiter (bez roli owner/admin) nie edytuje linków —
...
select pg_temp.assert((select count(*) = 0 from upd),
  'CL162-1 member nie edytuje stronę WWW firmy (RLS: zero wierszy)');
reset role; reset app.current_uid;
...
select pg_temp.assert((select count(*) = 0 from upd),
  'CL162-1b recruiter (bez owner/admin) nie edytuje logo firmy (RLS: zero wierszy)');
reset role; reset app.current_uid;
select pg_temp.assert(
  (select website is null and logo_url is null from public.companies where id = :'COMPCL'),
  'CL162-1c nieudane próby nie zmieniły danych');

-- CL162-2: http:// (nie-https) odrzucone przez CHECK, niezależnie od roli/ścieżki.
...
select pg_temp.expect_error(
  ...
  'companies_website_https', 'CL162-2 http:// odrzucone (strona WWW)');
select pg_temp.expect_error(
  'update public.companies set logo_url = ''javascript:alert(1)'' where id = ''e1620000-0000-0000-0000-0000000000f1''',
  'companies_logo_url_https', 'CL162-2b adres bez https:// odrzucony (logo)');
select pg_temp.expect_error(
  'update public.companies set website = ''https://exa mple.com'' where id = ''e1620000-0000-0000-0000-0000000000f1''',
  'companies_website_https', 'CL162-2c spacja w adresie odrzucona');
reset role; reset app.current_uid;

-- CL162-3: owner ustawia OBA adresy poprawnie → zapis, status BEZ ZMIAN (verified), audyt.
set role authenticated; set app.current_uid = :'OWNCL'; select pg_temp.assert_client_role();
...
select pg_temp.assert(
  ...
  'CL162-3 adresy zapisane, weryfikacja NIE cofnięta (w przeciwieństwie do nazwy/VAT)');
select pg_temp.assert(
  ...
  'CL162-3b audyt zmiany linków z wartościami przed/po');
select pg_temp.assert(
  ...
  'CL162-3c bez wpisu zmiany statusu — zmiana linków nie uruchamia ponownej weryfikacji');

-- CL162-4: admin (nie tylko owner) może edytować; puste pole czyści adres (NULL).
...
select pg_temp.assert(
  (select website = 'https://www.firma-cl.example' and logo_url is null and status::text = 'verified'
     from public.companies where id = :'COMPCL'),
  'CL162-4 admin czyści logo bez wpływu na stronę WWW ani status');

-- CL162-5: adres nad limitem długości (2048 znaków) odrzucony (SEC-04-style, path-independent).
set role authenticated; set app.current_uid = :'OWNCL'; select pg_temp.assert_client_role();
select pg_temp.expect_error(
  format('update public.companies set website = ''https://www.firma-cl.example/%s'' where id = ''e1620000-0000-0000-0000-0000000000f1''',
         repeat('a', 2048)),
  'companies_website_https', 'CL162-5 adres nad limitem długości odrzucony');
reset role; reset app.current_uid;

-- CL162-6: zmiana nazwy TEJ SAMEJ firmy nadal cofa weryfikację (bez regresji 0072/0162).
set role authenticated; set app.current_uid = :'OWNCL'; select pg_temp.assert_client_role();
...
select pg_temp.assert(
  ...
  'CL162-6 zmiana nazwy nadal cofa weryfikację — CL162 nie osłabił 0072');
