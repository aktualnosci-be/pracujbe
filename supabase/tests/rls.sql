...
-- MN131 (0131, numer tymczasowy): e-mail newMessage niesie tylko LICZBĘ załączników
...
select pg_temp.assert(
  ...
  'MN131-1 e-mail o wiadomości z plikiem niesie liczbę załączników (obie strony)');
select pg_temp.assert(
  not exists (select 1 from public.email_deliveries
  ...
  'MN131-2 payload bez nazw plików');
select set_config('app.current_uid', :'MAC', false);
set role authenticated; select pg_temp.assert_client_role();
...
select pg_temp.assert(
  ...
  'MN131-3 wiadomość bez pliku: attachmentCount = 0');
-- Kontrola ujemna do MN131-1: ta sama firma, ten sam typ pliku — różnica tylko w blokadzie.
select pg_temp.assert(
  ...
  'MN131-4 firma zablokowana przez nadawcę nie dostaje liczby jego plików');
