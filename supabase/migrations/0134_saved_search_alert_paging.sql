-- =============================================================================
-- 0134 (numer tymczasowy — ostateczny nada integrator; kolejka: 0134 #622, 0135 #653) — alerty zapisanych
...
--
-- Zmiana:
--   1. get_public_jobs: stały porządek remisów (`j.id desc` po `published_at desc`).
--      Bez tego strony z różnym offsetem mogły ułożyć oferty z tym samym published_at
--      inaczej (sort top-N nie jest stabilny) → dziura albo dubel na granicy strony.
--      Ta sama poprawka dotyczy paginacji listy /oferty-pracy. Poza ORDER BY treść
--      funkcji = 0110.
...
-- Rollback: przywróć get_public_jobs i process_saved_search_alerts z 0110 / 0092,
...
language sql stable security definer set search_path = public, pg_temp as $$
  ...
  order by
    ...
    j.published_at desc,
    j.id desc  -- 0134: remis published_at rozstrzygany stale (stronicowanie bez dziur i dubli)
...
begin
  ...
  loop
    ...
    -- 0134: wszystkie strony (nie tylko 100 najnowszych), jeden snapshot.
    with found as (
      select m.id
