...
Przed wdrożeniem zmian także na dotychczasowym hostingu ustaw APP_MODE=production w produkcji. Automatyczne wykrywanie przez VERCEL_ENV zostaje usunięte. Staging z prawdziwą bazą również wymaga APP_MODE=production i własnych sekretów. NEXT_PUBLIC_SITE_URL musi być prawdziwym adresem HTTPS; staging zachowuje noindex.
...
Staging: już tymczasowa domena Railway używana jako NEXT_PUBLIC_SITE_URL musi zawierać słowo staging lub preview (np. staging-pracujbe.up.railway.app). Losowa nazwa bez tego oznaczenia jest dziś klasyfikowana jako indeksowalna produkcja. Po dodaniu docelowej domeny ustaw https://staging.pracuj.be i przebuduj aplikację; sprawdź X-Robots-Tag, robots.txt i sitemap przed udostępnieniem.
