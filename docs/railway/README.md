...
`/api/maintenance` wywołuje co przebieg także `expire_due_jobs()` (migracja `0086`, tylko `service_role`). Operacja zmienia wyłącznie oferty `active` z ustawionym `expires_at <= now()` na `expired` i zwraca ich liczbę; szkic, wstrzymana, zamknięta, bez daty i z datą przyszłą zostają bez zmian. Jest idempotentna: równoległe lub ponowione wywołanie pomija rekordy zablokowane przez inny przebieg (`SKIP LOCKED`) i nie wysyła powiadomień ani e-maili.
...
- **Obserwowalność:** odpowiedź 200 zawiera tylko liczniki (`expiredJobs`, `releasedDiscounts`, `releasedCheckouts`) — bez danych ofert i bez sekretu; skrypt crona loguje sam kod HTTP. Błąd któregokolwiek zadania → 503 i zdarzenie Sentry `maintenance.gc` z polem `task` (`jobExpiry` dla wygaszania), a cron kończy się kodem 1 (nieudane wykonanie w Railway). Brak service-role w produkcji → 503 `unconfigured`.
