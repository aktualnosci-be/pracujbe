...
`0108` nie ma sekcji `mail`: czujki poczty milczą, reszta działa. Kolejka auth nie
...
| **Schemat** (`0108`, poczta) | NOWA migracja naprawcza z ciałem `ops_metrics()` z `0096` i `drop index public.idx_email_deliveries_sent_at` | aplikacja toleruje brak sekcji `mail` (czujki poczty milczą) |
