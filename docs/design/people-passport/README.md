...
Wierność prototypowi (#5/#7): strona główna, nagłówek i karty ofert porównane zrzutami 1280 i 390 px z `#people/home` i `#people/jobs`. Font DM Sans (`next/font/local`, podzbiór ~42 KB, SIL OFL 1.1). Nagłówek: pogrubione czarne linki, kod języka, „Zaloguj się” i czarny przycisk „Dodaj ofertę” (cel bez zmian). Hero: trzeci wiersz w `--primary` (≈ #D92932, 4,9:1 z bielą), drugi wybór jako zwykły link tekstowy, fotografia z promieniem 100/22 px. Wyszukiwarka w jednym kontenerze z etykietami nad polami bez ramek i separatorem; fokus widoczny na całej komórce pola. „Najnowsze oferty” na szarym tle z liniami, z podtytułem i czarnym „Zobacz wszystkie oferty →”; karty w siatce 2 kolumn od `lg` — tyle, ile faktycznie renderuje zatwierdzony prototyp (`conditions.css` nadpisuje trzy kolumny z `people.css`); lista z filtrami zostaje w jednej kolumnie, także zgodnie z prototypem. Karta-paszport: jedna ramka i promień 20 px (usunięty wrapper `divide-y`, który dawał podwójne krawędzie), zakładka bez obramowanego kwadratu, stopka z logotypem i „Poznaj ofertę →”, data publikacji przy nazwie firmy. Kafelki „Utwórz profil / Dodaj ofertę” i pasek zaufania przeniesione pod listę ofert (prototyp nie ma ich w hero; funkcje i cele linków bez zmian), w stylu `p-profile-note`. Oznaczenia trybu demo zostają. Pozostaje: wyszukiwarka na liście ofert (osobne pola z ramkami) — do wyrównania po scaleniu zmian filtrów (#471).
