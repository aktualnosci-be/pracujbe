...
| `security-audit` | Bezpieczeństwo, audyt, limity, logi | Dziennik audytu z IP/User-Agent i kopiami pól, klucze limitera, zdarzenia systemowe, kody błędów | Administratorzy | Railway, Supabase, Discord (webhook błędów) | Brak dla audytu; limiter (`rate_limit_gc`, okna > 1 doby) i inbox webhooków (`processed_webhooks_gc`, rozstrzygnięte > 30 dni) czyści `/api/maintenance` (0193) | Wiadomość webhooka błędów tylko z kodem i trasą (#571), HMAC w limiterze PostgreSQL | do ustalenia | właściciel + prawnik |
| `ai-job-import` | Import ogłoszenia przez AI | Tekst ogłoszenia po redakcji kontaktów i identyfikatorów albo zrzut ekranu bez redakcji | Pracodawca (szkic) | OpenAI (od 2026-09-26, wcześniej Anthropic), Railway, Supabase | Portal nie zapisuje wejścia, tylko szkic | Flaga domyślnie wyłączona, limity per firma, bez publikacji | do ustalenia — patrz [dostawcy-i-transfery.md](dostawcy-i-transfery.md) | właściciel + prawnik |
