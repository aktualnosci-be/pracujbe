...
| `security-audit` | Bezpieczeństwo, audyt, limity, logi | Dziennik audytu z IP/User-Agent i kopiami pól, klucze limitera, zdarzenia systemowe, kody błędów | Administratorzy | Railway, Supabase, Discord (webhook błędów) | Brak dla audytu i limitera; `processed_webhooks_gc` istnieje, ale nie jest wywoływana | Wiadomość webhooka błędów tylko z kodem i trasą (#571), HMAC w limiterze PostgreSQL | do ustalenia | właściciel + prawnik |
