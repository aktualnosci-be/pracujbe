...
async function expectOfferCards(page: Page, locale: (typeof locales)[number]) {
  ...
  await expect(first.getByText('Liège')).toBeVisible();
  ...
  await expect(first.locator('dd').nth(0)).toHaveText('12');
  await expect(first.locator('dd').nth(1)).toHaveText('6');
