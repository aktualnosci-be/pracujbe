...
import { getPublicJob, getPublicJobs, getPublicJobsCount } from '../../src/lib/db/public-jobs';
...
const visibleSlugs = ['warehouse-rich', 'transport', 'without-salary', 'fallback-default', 'fallback-en', 'canonical'];
const hiddenSlugs = ['draft', 'expired-date', 'expired-status', 'deleted-job', 'unverified-company', 'deleted-company'];
...
beforeAll(async () => {
  ...
  docker('run', '--detach', '--rm', '--name', container, '--publish', '127.0.0.1::5432',
    ...
    '--env', 'POSTGRES_DB=public_jobs_test', 'postgres:16');
  created = true;
  ...
  admin = new Pool({ host: '127.0.0.1', port, password, database: 'public_jobs_test', user: 'postgres', max: 1,
  ...
  app = await createRuntimePool(`postgresql://public_jobs_web:${password}@127.0.0.1:${port}/public_jobs_test`, 'domain');

  await admin.query(`INSERT INTO public.companies(id,name,status,description,email,phone,address,deleted_at)
    VALUES ($1,'Firma publiczna','verified','Publiczny opis firmy','private@example.invalid','private-phone','private-address',null),
      ...
      ($3,'Firma usunięta','verified',null,null,null,null,now())`, [companyId, unpublishedCompanyId, deletedCompanyId]);

  const fixtures = [
    { slug: 'warehouse-rich', category: 'warehouse', contract: 'permanent', city: 'Antwerp', min: 3000, max: 3500,
      accommodation: true, immediate: true, noLanguage: true, locale: 'nl' },
    { slug: 'transport', category: 'transport', contract: 'temporary', city: 'Ghent', min: 4000, max: 4500 },
    { slug: 'without-salary', category: 'construction', contract: 'interim', city: 'Antwerp',
      accommodation: true, noLanguage: true },
    { slug: 'fallback-default', category: 'warehouse', contract: 'permanent', city: 'Brussels', min: 2500, max: 2700, locale: 'nl' },
    { slug: 'fallback-en', category: 'cleaning', contract: 'permanent', city: 'Leuven', min: 2000, max: 2200, locale: 'fr' },
    { slug: 'canonical', category: 'technical', contract: 'permanent', city: 'Leuven', min: 1800, max: 2100 },
  ];
  for (const [index, fixture] of fixtures.entries()) {
    await admin.query(`INSERT INTO public.jobs(id,company_id,slug,title,status,category,contract_type,city,region,
    ...
    [index === 0 ? richJobId : randomUUID(), companyId, fixture.slug, `Tytuł bazowy ${fixture.slug}`, fixture.category,
      fixture.contract, fixture.city, fixture.min ?? null, fixture.max ?? null, daysAgo(index + 1), expiresAt,
      fixture.accommodation ?? false, fixture.immediate ?? false, fixture.noLanguage ?? false, fixture.locale ?? 'pl']);
  }
  for (const slug of hiddenSlugs) {
    await admin.query(`INSERT INTO public.jobs(company_id,slug,title,status,category,contract_type,city,region,
      published_at,expires_at,deleted_at)
      VALUES ($1,$2,'Oferta ukryta',$3,'warehouse','permanent','Antwerp','Flandria',$4,$5,$6)`,
    [slug === 'unverified-company' ? unpublishedCompanyId : slug === 'deleted-company' ? deletedCompanyId : companyId,
      slug, slug === 'draft' ? 'draft' : slug === 'expired-status' ? 'expired' : 'active', daysAgo(0),
      slug === 'expired-date' ? daysAgo(1) : expiresAt, slug === 'deleted-job' ? daysAgo(1) : null]);
  }

  for (const [locale, title] of Object.entries({ pl: 'Magazynier PL', nl: 'Magazijn NL', fr: 'Magasin FR', en: 'Warehouse EN' })) {
    await admin.query(`INSERT INTO public.job_translations(job_id,locale,title,description,responsibilities,
      conditions,highlights,company_description)
      VALUES ($1,$2,$3,$4,ARRAY[$5],ARRAY[$6],ARRAY[$7],$8)`,
    [richJobId, locale, title, `Opis ${locale}`, `Obowiązek ${locale}`, `Warunek ${locale}`, `Atut ${locale}`, `Firma ${locale}`]);
  }
    ...
    try { await admin?.end(); }
      ...
      if (created) {
        for (let attempt = 0; attempt < 3; attempt++) {
          try { docker('rm', '--force', container); } catch { /* sprawdzamy własną nazwę poniżej */ }
...
describe('Publiczne oferty — pełne migracje i rzeczywisty PostgreSQL 16', () => {
  it('pokazuje wyłącznie aktywne niewygasłe oferty zweryfikowanych istniejących firm', async () => {
    ...
    for (const slug of hiddenSlugs) expect(await getPublicJob(app!, slug, 'pl')).toBeNull();
  });

  it.each([['pl', 'Magazynier PL'], ['nl', 'Magazijn NL'], ['fr', 'Magasin FR'], ['en', 'Warehouse EN'],
    ['unsupported', 'Magazynier PL']])('zachowuje język %s listy i detalu', async (locale, title) => {
    const result = await getPublicJobs(app!, { locale });
    expect(result.rows[0]?.title).toBe(title);
    expect((await getPublicJob(app!, 'warehouse-rich', locale))?.title).toBe(title);
  });
  ...
  it('stosuje fallback do języka oferty, potem angielskiego i tytułu bazowego', async () => {
    ...
    for (const [slug, title] of [['fallback-default', 'Domyślny NL'], ['fallback-en', 'Fallback EN'],
    ...
    expect((await getPublicJob(app!, 'warehouse-rich', 'fr'))?.requirements_mandatory).toEqual(['Wymaganie NL']);
  });

  const filters: [string, Partial<GetJobsParams>, string[]][] = [
    ['kategoria pojedyncza', { category: 'transport' }, ['transport']],
    ['kategorie', { categories: ['warehouse'] }, ['warehouse-rich', 'fallback-default']],
    ['pierwszeństwo tablicy', { category: 'warehouse', categories: ['cleaning'] }, ['fallback-en']],
    ['umowa pojedyncza', { contractType: 'temporary' }, ['transport']],
    ['umowy', { contractTypes: ['interim'] }, ['without-salary']],
    ['miasta', { locations: ['Antwerp'] }, ['warehouse-rich', 'without-salary']],
    ['fragment miasta', { city: 'Ant' }, ['warehouse-rich', 'without-salary']],
    ['słowo', { keyword: 'magazynier' }, ['warehouse-rich']],
    ['słowo w języku odbiorcy', { locale: 'en', keyword: 'warehouse' }, ['warehouse-rich']],
    ['przedział pensji', { salaryMin: 3200, salaryMax: 3600 }, ['warehouse-rich', 'without-salary']],
    ['pensja nie wyklucza niepodanej', { salaryMin: 5000 }, ['without-salary']],
    ['pensja maksymalna', { salaryMax: 2200 }, ['without-salary', 'fallback-en', 'canonical']],
    ['zakwaterowanie', { accommodation: true }, ['warehouse-rich', 'without-salary']],
    ['bez zakwaterowania', { accommodation: false }, ['transport', 'fallback-default', 'fallback-en', 'canonical']],
    ['od zaraz', { immediate: true }, ['warehouse-rich']],
    ['false nie filtruje od zaraz', { immediate: false }, visibleSlugs],
    ['bez języka', { noLanguageRequired: true }, ['warehouse-rich', 'without-salary']],
    ['data', { since: daysAgo(2.5) }, ['warehouse-rich', 'transport']],
    ['razem', { categories: ['warehouse'], contractTypes: ['permanent'], locations: ['Antwerp'],
      salaryMin: 3000, salaryMax: 3500, accommodation: true, immediate: true, noLanguageRequired: true,
      since: daysAgo(1.5), keyword: 'Magazynier', city: 'Ant' }, ['warehouse-rich']],
    ['brak wyników', { keyword: 'nieistniejący tytuł' }, []],
  ];
  it.each(filters)('filtr %s daje ten sam wynik listy i licznika', async (_label, filter, slugs) => {
  ...
  it('sortuje i stronicuje w SQL, a licznik obejmuje również pozostałe strony', async () => {
    ...
    expect(salary.rows.map(row => row.slug)).toEqual(['transport', 'warehouse-rich', 'fallback-default', 'fallback-en', 'canonical', 'without-salary']);
    const page = await getPublicJobs(app!, { locale: 'pl', page: 2, pageSize: 2 });
    expect(page.rows.map(row => row.slug)).toEqual(['without-salary', 'fallback-default']);
    expect(page).toMatchObject({ total: 6, page: 2, pageSize: 2 });
    expect(await getPublicJobs(app!, { locale: 'pl', page: 4, pageSize: 2 })).toMatchObject({ rows: [], total: 6 });
  });

  it('normalizuje nieprawidłową paginację i nakłada sufit SQL bez przepełnienia', async () => {
    expect(await getPublicJobs(app!, { locale: 'pl', page: Number.NaN, pageSize: Number.POSITIVE_INFINITY }))
      .toMatchObject({ page: 1, pageSize: 12, total: 6 });
    expect(await getPublicJobs(app!, { locale: 'pl', page: -2, pageSize: -3 })).toMatchObject({ page: 1, pageSize: 1 });
    expect(await getPublicJobs(app!, { locale: 'pl', page: 1.9, pageSize: 1000 })).toMatchObject({ page: 1, pageSize: 100 });
    expect(await getPublicJobs(app!, { locale: 'pl', page: Number.MAX_VALUE, pageSize: 100 }))
  ...
  it('zachowuje kontrakt JSON dat, liczb, null i tablic oraz nie ujawnia prywatnych pól', async () => {
    ...
    expect(detail).toMatchObject({ salary_min: 3000, salary_max: 3500, salary_period: 'hour',
      start_date: '2026-10-01', description: 'Opis pl', responsibilities: ['Obowiązek pl'], conditions: ['Warunek pl'],
      requirements_mandatory: ['Wymaganie PL'], requirements_optional: [], languages: ['Niderlandzki'],
      transport: true, company_description: 'Firma pl', working_hours: '40 godzin', shifts: 'Dzienna' });
    expect(typeof detail?.published_at).toBe('string');
    expect(typeof detail?.expires_at).toBe('string');
    expect(Date.parse(detail!.published_at as string)).toBe(Date.parse(daysAgo(1)));
    ...
    for (const row of [...list.rows, detail!]) {
      ...
      for (const field of ['company_id', 'created_by', 'contact_email', 'email', 'phone', 'address', 'vat_number',
        'status', 'created_at', 'updated_at', 'views_count', 'applications_count']) expect(row).not.toHaveProperty(field);
    }
  });
  ...
  it('przekazuje teksty i tablice jako parametry, także gdy wyglądają jak SQL', async () => {
    ...
    for (const filter of [{ keyword: input }, { city: input }, { locations: [input] },
      { categories: [input] }, { contractTypes: [input] }]) {
      const params = { locale: 'pl', ...filter } as GetJobsParams;
      expect(await getPublicJobs(app!, params)).toMatchObject({ rows: [], total: 0 });
      expect(await getPublicJobsCount(app!, params)).toBe(0);
    }
    expect((await getPublicJobs(app!, { locale: input })).rows[0]?.title).toBe('Magazynier PL');
    expect((await admin!.query('SELECT count(*)::int AS count FROM public.jobs')).rows[0]?.count).toBe(12);
  });
  ...
  it('wykonuje każdy odczyt jako anon z pustą tożsamością i nie daje dostępu do tabel bazowych', async () => {
    ...
    const checkedPool: TransactionPool = { connect: async () => {
    ...
    expect(observed.every(state => state.role === 'anon' && state.uid === null)).toBe(true);
    await expect(app!.query('SELECT * FROM public.jobs')).rejects.toMatchObject({ code: '42501' });
  ...
  it('propaguje awarię RPC zamiast podstawiać pustą listę lub dane demo', async () => {
    await admin!.query('REVOKE EXECUTE ON FUNCTION public.get_public_job(text,text) FROM anon');
    try { await expect(getPublicJob(app!, 'warehouse-rich', 'pl')).rejects.toMatchObject({ code: '42501' }); }
    finally { await admin!.query('GRANT EXECUTE ON FUNCTION public.get_public_job(text,text) TO anon'); }
    expect((await getPublicJob(app!, 'warehouse-rich', 'pl'))?.id).toBe(richJobId);
  });
});
