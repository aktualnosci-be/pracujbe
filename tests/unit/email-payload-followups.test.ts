 ...
 * Kontrola ujemna: te same asercje na migracjach bez 0113 (stan sprzed zmiany) padają.
 */
...
describe('0113: klucze payloadu z migracji', () => {
  ...
  it('kontrola ujemna: migracje bez 0113 nie spełniają kontraktu', () => {
    expect(missingKeys(withoutFollowup)).toEqual([...OFFER_KEYS.map((k) => `jobOffer.${k}`), 'newMessage.conversationId']);
  });
});

describe('0113: worker formatuje w locale odbiorcy', () => {
